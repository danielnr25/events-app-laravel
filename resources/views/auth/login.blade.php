<x-layouts.guest title="Login">
    ...
    <div class="flex min-h-full flex-col justify-center px-6 py-12 lg:px-8">
        <div class="sm:mx-auto sm:w-full sm:max-w-md">
            <div class="w-full max-w-md p-4 bg-white border border-gray-200 rounded-lg shadow-md sm:p-6 md:p-8 dark:bg-gray-800 dark:border-gray-700">
                <form class="space-y-6" action="{{ route('login') }}" method="POST">
                    @csrf
                    <img class="mx-auto h-12 w-auto" src="https://tailwindui.com/img/logos/workflow-mark-blue-600.svg" alt="Workflow">
                    <h2 class="mt-4 text-center text-2xl font-bold leading-9 tracking-wide text-gray-900">Ingrese sus credenciales</h2>
                    @if(session('message'))
                    <p class="text-white bg-red-500 py-2 my-2 rounded-lg text-sm text-center">{{ session('message') }}</p>
                    @endif
                    <div>
